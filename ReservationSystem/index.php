...
<head>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Inicializar Alpine Store para el modal de cancelación
            document.addEventListener('alpine:init', () => {
                Alpine.store('cancelModalData', {
                    reservaId: '',
                    reservaInfo: '',
                    telefono: '',
                    tipo_telefono: '',
            ...
            window.addEventListener('populate-cancel', function(event) {
                if (window.Alpine && Alpine.store) {
                    Alpine.store('cancelModalData').updateData(event.detail);
                }
            });
        });
    </script>
    ...
    <div x-show="showCancelModal" class="fixed inset-0 overflow-y-auto z-50 flex items-center justify-center" style="display: none;">
        ...
        <div class="relative bg-gray-800 rounded-xl max-w-md w-full mx-4 shadow-2xl border border-gray-700"
            x-data='{ 
                ...
                avisado: false,
                init() {
                    this.$watch("$store.cancelModalData.cargando", (val) => {
                        if (val === true && this.$store.cancelModalData.reservaId) {
                            this.cargarContacto(this.$store.cancelModalData.reservaId);
                        }
                    });
                },
                abrirWsp() {
                    ...
                    let telefono = this.$store.cancelModalData.telefono;
                    let nombreapellido = this.$store.cancelModalData.nombreapellido;
                    let reservaInfo = this.$store.cancelModalData.reservaInfo;
                    
                    let cleanPhone = telefono.replace(/[^0-9]/g, "");
                    if (cleanPhone.startsWith("0")) cleanPhone = cleanPhone.substring(1);
                    
                    let msg = `¡Hola ${nombreapellido}! 🏫 Te escribimos desde el *Sistema de Reservas de la E.E.S.T. N° 1 "Mariano Moreno"* de Chivilcoy.\n\nTe queríamos avisar que tuvimos que cancelar tu reserva de *${reservaInfo}*.\n\n📌 *Motivo:* ${this.motivo}\n\nCualquier duda, podés ingresar de nuevo a la web para pedir un cambio de fecha o realizar otra reserva. ¡Disculpas por las molestias! 💻\n\n¡Que tengas un buen día! 👋`;
                    
                    window.open("https://wa.me/549" + cleanPhone + "?text=" + encodeURIComponent(msg), "_blank");
                    this.$store.cancelModalData.avisado = true;
                },
                cargarContacto(id) {
                    this.$store.cancelModalData.cargando = true;
                    fetch("get_reserva_contacto.php?id=" + id)
                        .then(r => r.json())
                        .then(data => {
                            this.$store.cancelModalData.telefono = data.telefono || "";
                            this.$store.cancelModalData.tipo_telefono = data.tipo_telefono || "";
                            this.$store.cancelModalData.nombreapellido = data.nombreapellido || this.$store.cancelModalData.nombreapellido;
                            this.$store.cancelModalData.avisado = (data.telefono === "");
                            this.$store.cancelModalData.cargando = false;
                        })
                        .catch((error) => {
            ...
            <form action="reserva/cancelar_reserva_sql.php" method="POST" class="p-6">
                <input type="hidden" name="id" x-model="$store.cancelModalData.reservaId">
                <input type="hidden" name="info" x-model="$store.cancelModalData.reservaInfo">
                
                <p class="text-gray-300 text-sm mb-4">Vas a cancelar la reserva <strong x-text="$store.cancelModalData.reservaInfo" class="text-white"></strong>. Por favor, indica el motivo para notificar al usuario (si corresponde).</p>
                ...
                <div class="mb-4 p-3 bg-gray-700/50 rounded-lg border border-gray-600 text-sm">
                    ...
                    <div x-show="$store.cancelModalData.cargando" class="flex items-center justify-center py-3">
                    ...
                    <div x-show="!$store.cancelModalData.cargando && $store.cancelModalData.telefono">
                        <p class="text-gray-400">Usuario: <span class="text-white" x-text="$store.cancelModalData.nombreapellido"></span></p>
                        <p class="text-gray-400">Teléfono: <span class="text-white" x-text="$store.cancelModalData.telefono"></span> (<span class="text-white" x-text="$store.cancelModalData.tipo_telefono === 'whatsapp' ? 'WhatsApp' : ($store.cancelModalData.tipo_telefono === 'celular_sin_wsp' ? 'Celular sin WhatsApp' : 'Línea Fija')"></span>)</p>
                        
                        <div x-show="$store.cancelModalData.tipo_telefono === 'whatsapp'" class="mt-2 text-green-400 flex flex-col space-y-2">
                        ...
                        <div x-show="$store.cancelModalData.tipo_telefono !== 'whatsapp'" class="mt-2 text-amber-400 flex flex-col space-y-1">
                            <div class="flex items-center text-xs">
                            ...
                            <label class="flex items-center space-x-2 text-xs text-gray-300 mt-1 cursor-pointer bg-gray-700/30 p-2 rounded border border-gray-600/30">
                                <input type="checkbox" x-model="$store.cancelModalData.avisado" class="rounded text-amber-500 focus:ring-amber-500 bg-gray-700 border-gray-600 h-4 w-4">
                ...
                <div class="flex justify-end space-x-3">
                    ...
                    <button type="submit" :disabled="!$store.cancelModalData.avisado" :class="$store.cancelModalData.avisado ? 'bg-red-600 hover:bg-red-700 text-white cursor-pointer' : 'bg-red-800 text-red-400 cursor-not-allowed opacity-50'" class="px-4 py-2 rounded-lg transition-all">Confirmar Cancelación</button>
                </div>
            </form>
        </div>
    </div>
